...
I believe that we are all born with a gift. It is our job to find it and give it away.
I believe that we are all born with a gift. It is our job to find it and give it away. I believe that we are all born with a gift. It is our job to find it and give it away.
I believe that we are"""
  ...
  generate_ids = model.generate(input_ids, max_length=100)
  answer_output = tokenizer.batch_decode(generate_ids, skip_special_tokens=True, clean_up_tokenization_spaces=False)[0]
  end = datetime.now()
  print("------end LLaMA 7B inference------")
  print(f'The inference time of LLaMA 7B on the current device is: {(end - start).total_seconds():.2f} s', flush=True)
  print("prompt: \n", prompt)
...
if __name__ == "__main__":
    # LLaMA 7B inference
    parser = argparse.ArgumentParser(description="Script for LLaMA 7B inference")
    parser.add_argument("--model_path", type=str, default="/mnt/lustre/share_data/PAT/datasets/llama_7B_hf")
    args = parser.parse_args()
    model = AutoModelForCausalLM.from_pretrained(args.model_path, torch_dtype=torch.float16, low_cpu_mem_usage=True)
